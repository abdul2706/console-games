...
var tanks = [
    new Tank(CONFIG.CONSOLE_WIDTH / 2, CONFIG.CONSOLE_HEIGHT / 2, DIRECTION.UP),
    new Tank(1, 1, DIRECTION.UP),
    new Tank(CONFIG.CONSOLE_WIDTH - 2, 1, DIRECTION.UP),
    new Tank(1, CONFIG.CONSOLE_HEIGHT - 2, DIRECTION.UP),
    new Tank(CONFIG.CONSOLE_WIDTH - 2, CONFIG.CONSOLE_HEIGHT - 2, DIRECTION.UP),
];
var bullets = [];
...
function mouseClicked() {
    ...
    if (CONFIG.GAME_STATUS == GAME_STATUSES.START) {
        ...
        if (mouseX > 3 * CONFIG.UNIT_PIXEL_WIDTH &&
            mouseX < 17 * CONFIG.UNIT_PIXEL_WIDTH &&
        ...
        clog('game status is end');
        if (mouseX > 0 &&
            mouseX < 2 * CONFIG.UNIT_PIXEL_WIDTH &&
            mouseY > CONFIG.CONSOLE_HEIGHT * CONFIG.UNIT_PIXEL_HEIGHT) {
            clog('game status changing to start');
            CONFIG.GAME_STATUS = GAME_STATUSES.START;
            clear_game();
        }
    }
}
...
function keyPressed() {
    ...
    if (keyCode == 32) {
        bullets.push(tanks[0].create_bullet());
    } else if (keyCode == 27) {
        if (CONFIG.GAME_STATUS == GAME_STATUSES.PLAY) {
            CONFIG.GAME_STATUS = GAME_STATUSES.PAUSE;
        } else if (CONFIG.GAME_STATUS == GAME_STATUSES.PAUSE) {
...
function update_user_tank() {
    ...
    let current_direction = tanks[0].direction;
    let x_prev = tanks[0].x,
        y_prev = tanks[0].y,
        d_prev = tanks[0].direction;

    if (keyIsDown(UP_ARROW) || keyIsDown(87)) {
        // clog(TAG2 + '[UP_ARROW]');
        if (current_direction == DIRECTION.UP) {
            tanks[0].setY(tanks[0].y - CONFIG.GAME_SPEED);
        } else {
            tanks[0].direction = DIRECTION.UP;
        }
    } else if (keyIsDown(RIGHT_ARROW) || keyIsDown(68)) {
        // clog(TAG2 + '[RIGHT_ARROW]');
        if (current_direction == DIRECTION.RIGHT) {
            tanks[0].setX(tanks[0].x + CONFIG.GAME_SPEED);
        } else {
            tanks[0].direction = DIRECTION.RIGHT;
        }
        ...
        if (current_direction == DIRECTION.DOWN) {
            ...
            tanks[0].direction = DIRECTION.DOWN;
        }
        ...
        if (current_direction == DIRECTION.LEFT) {
            ...
            tanks[0].direction = DIRECTION.LEFT;
        }
    }

    let collision = false;
    for (let i = 1; i < tanks.length; i++) {
        if (tanks[0].check_tank_collision(tanks[i])) {
            collision = true;
            break;
        }
    }
    ...
    for (let i = 0; i < bullets.length; i++) {
        ...
        for (let j = 0; j < tanks.length; j++) {
            ...
            if (Math.abs(x1 - x2) < 2 && Math.abs(y1 - y2) < 2) {
                ...
                if (j == 0) {
                    CONFIG.GAME_STATUS = GAME_STATUSES.END;
                    return;
                } else {
                    if (bullets[i].parent_tank == tanks[0]) {
                        CONFIG.USER_SCORE++;
                        bullets.splice(i, 1);
                        tanks.splice(j, 1);
                    }
